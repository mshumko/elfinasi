...
events = [
    {
        # From Artemyev+2025---ELFIN is too far east.
...
for event in events:
    ...
    state_name = (
        f'{dateutil.parser.parse(event["time_range"][0]).strftime("%Y%m%d_%H%M%S")}_'
        f'{dateutil.parser.parse(event["time_range"][1]).strftime("%H%M%S")}_'
        f'elfin{event["sc_id"].lower()}_{alt}_km_mapped_state.csv'
        )
    mapped_state.to_csv(elfinasi.data_dir / state_name, index_label='time')
